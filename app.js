...
const planTableBody = $("planTable").querySelector("tbody");
...
/* Monthly view */
const monthlyTableBody = $("monthlyTable").querySelector("tbody");
...
const catsTableBody = $("catsTable").querySelector("tbody");
...
const recTableBody = $("recTable").querySelector("tbody");
...
const billsTableBody = $("billsTable").querySelector("tbody");
...
const debtsTableBody = $("debtsTable").querySelector("tbody");
...
const vehiclesTableBody = $("vehiclesTable").querySelector("tbody");
...
btnRefreshCategories.addEventListener("click", async () => {
  await loadCategories();
  catStatus.textContent="Refreshed.";
});
...
async function loadCategories() {
  ...
  snap.forEach(d=>{
    const x=d.data()||{};
    list.push({ id:d.id, group:String(x.group||"").trim(), name:String(x.name||"").trim(), type:(x.type==="income"?"income":"expense") });
  });
...
btnAddRecurring.addEventListener("click", async ()=>{
  ...
  if (!group || !category) { recStatus.textContent="Please fill Group and Category."; return; }
  if (!startMonth) { recStatus.textContent="Please pick a start month."; return; }
  ...
  recAmount.value=""; recDay.value=""; recEnd.value="";
  await loadRecurring();
  await applyAutoStuffForMonth(currentMonthYM);
  await loadMonthData();
});
...
/* =========================
   Bills (checklist + auto create tx)
========================= */
btnAddBill.addEventListener("click", async ()=>{
  ...
  if (!name) { billStatus.textContent="Please enter bill name."; return; }
  if (!amount || amount<=0) { billStatus.textContent="Please enter amount."; return; }

  const { billsCol } = userRoot(currentUser.uid);
  await addDoc(billsCol, { name, group, amount, dueDay, active:true, createdAt:Date.now() });

  billName.value=""; billAmount.value=""; billDueDay.value="";
  billStatus.textContent="Saved.";
...
/* =========================
   Debts & Loans (auto interest/principal)
========================= */
btnAddDebt.addEventListener("click", async ()=>{
  ...
  if (!name) { debtStatus.textContent="Please enter debt name."; return; }
  if (!balance || balance<=0) { debtStatus.textContent="Please enter current balance."; return; }
  if (!monthly || monthly<=0) { debtStatus.textContent="Please enter monthly payment."; return; }
  ...
  debtName.value=""; debtApr.value=""; debtBalance.value=""; debtMonthlyPay.value=""; debtDueDay.value="";
  debtStatus.textContent="Saved.";
...
btnRefreshDebts.addEventListener("click", async ()=>{
  await loadDebts();
  debtStatus.textContent="Refreshed.";
});
...
function renderDebtsTable() {
  ...
  for (const d of debts) {
    ...
    tr.innerHTML=`
      ...
      <td>
        <button class="iconBtn pay" type="button">Pay this month</button>
      </td>
      <td>
...
btnAddFund.addEventListener("click", async ()=>{
  ...
  if (!name) { fundStatus.textContent="Please enter fund name."; return; }
  if (!goal || goal<=0) { fundStatus.textContent="Please enter goal."; return; }
  if (!target) { fundStatus.textContent="Please choose target month."; return; }
  ...
  fundName.value=""; fundGoal.value=""; fundTarget.value=""; fundSaved.value="";
  fundStatus.textContent="Saved.";
...
btnRefreshFunds.addEventListener("click", async ()=>{
  await loadFunds();
  fundStatus.textContent="Refreshed.";
});
...
btnAddVehicle.addEventListener("click", async ()=>{
  ...
  if (!name) { vehStatus.textContent="Please enter vehicle name."; return; }
  ...
  vehName.value=""; vehPlate.value=""; vehMileage.value=""; vehIntervalKm.value=""; vehIntervalMonths.value="";
...
btnRefreshVehicles.addEventListener("click", async ()=>{
  await loadVehicles();
  await loadMaintenanceLogs();
  vehStatus.textContent="Refreshed.";
});
...
function renderVehiclesTable() {
  ...
  for (const v of vehicles) {
    ...
    const nextKm = (lastMileage!=null) ? (lastMileage + Number(v.serviceIntervalKm||0)) : (Number(v.currentMileage||0) + Number(v.serviceIntervalKm||0));
    const nextDate = (lastDate) ? addMonthsToISO(lastDate, v.serviceIntervalMonths||6) : null;
...
btnSavePlan.addEventListener("click", async ()=>{
  ...
  planSavedHint.textContent = `Saved at ${new Date().toLocaleString()}`;
  await loadMonthData();
});
...
async function loadTransactionsForMonth(ym) {
  ...
  const q = query(txCol, where("effectiveMonth","==",ym), orderBy("date","asc"));
...
function renderMonthlyView() {
  ...
  monthlyTotalsHint.textContent = `Planned result: ${fmtRM(plannedIncome-plannedExpense)} • Actual result: ${fmtRM(actualIncome-actualExpense)}`;
...
async function renderOverview() {
  ...
  kpiPlannedNet.textContent = fmtRM(planNet);
...
function renderBarChart(planIncome, planExpense, actualIncome, actualExpense) {
  ...
  charts.bar = new Chart(ctx, {
    ...
    options:{ responsive:true, plugins:{legend:{display:true}}, scales:{ y:{ ticks:{ callback:(v)=>"RM "+v } } } }
  });
}
...
function renderLineChart(points) {
  ...
  charts.line=new Chart(ctx,{
    ...
    options:{ responsive:true, scales:{ y:{ ticks:{ callback:(v)=>"RM "+v } } } }
  });
}
...
========================= */
btnExportPdf.addEventListener("click", exportMonthlyPdf);
btnExportPdf2.addEventListener("click", exportMonthlyPdf);
